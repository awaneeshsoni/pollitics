...
function PollRoom({ roomCode, pollState, userName, votedOption, onVote }) {
    ...
    return (
        <div className="w-full max-w-3xl bg-white p-6 md:p-8 rounded-xl shadow-lg border border-gray-200">
            ...
            <div className={`text-center text-3xl font-bold mb-6 py-2 px-4 rounded-md ${timer <= 10 && timer > 0 && isVotingActive ? 'text-red-600 bg-red-100 animate-pulse' : 'text-gray-800 bg-gray-100'}`}>
                Time Left: <span className="tabular-nums">{timer}s</span>
            </div>

            <h3 className="text-2xl md:text-3xl font-bold text-center text-gray-800 mb-8 break-words">
                {question}
            </h3>
                        ...
                        <div key={option} className={`border rounded-lg p-4 transition-all duration-300 ease-in-out ${hasVotedForThis ? 'border-green-400 bg-green-50 shadow-md' : 'border-gray-200 bg-gray-50 hover:shadow-sm'}`}>
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                <button
                                    ...
                                    className={`w-full sm:w-auto flex-shrink-0 px-5 py-3 text-base font-medium rounded-md transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 whitespace-nowrap
                                ...
                                <div className="flex-grow w-full mt-3 sm:mt-0 sm:ml-4">
                                    ...
                                    <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                                        <div
                                            className={`h-3 rounded-full transition-all duration-500 ease-out ${hasVotedForThis ? 'bg-green-500' : 'bg-blue-500'}`}
